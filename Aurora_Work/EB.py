...
def Graphing_Ratio(space_craft_with_E, efield, bfield, time_E, time_B, query_dict, fig, axes):
    ...
    def Logic_for_one_step(index, index_satellite,Bresamp):
        ...
        frequencies_B_0, powerspec_B_0 = signal.periodogram(Bresamp[index_satellite][range(index_start, index_end), 0]*1e-9, 16, window="hann",detrend='linear', scaling='spectrum')

        frequencies_E_1, powerspec_E_1 = signal.periodogram(efield[index_satellite][range(index_start, index_end), 1]*1e-3, 16, window="hann",detrend='linear', scaling='spectrum')
                ...
                for l, val in enumerate(query_dict["B_periodogram"]):
                    for k, val_sat in enumerate((query_dict["satellite_graph"])):
                        if val == 'B_North':
                            ...
                            index=1
                        axes_ani[axes_used + l ].plot(data[k, i, 0, 0, : ], data[k, i, 2,index, :]*1e9, label=val_sat) #Frequencies E's
                ...
                for l, val in enumerate(query_dict["E_periodogram"]):
                    for k, val_sat in enumerate((query_dict["satellite_graph"])):
                        if val == 'E_North':
                            ...
                            index=1
                        axes_ani[axes_used + l ].plot(data[k, i, 0, 0, : ], data[k, i, 1,index, :]*1e3, label=val_sat) #Frequencies E's
            ...
            for k in range(len(query_dict["satellite_graph"])):
                if query_dict['coordinate_system'][0] == "North East Centre":
                    ...
                    if query_dict["heatmap"][i] == "E_Azimuth":
                        ...
                        index2=  1
                if index1==3:
                    img = axes[2*i+k + length_for_axis].pcolormesh(datetimes[np.array(data[k, :, 4,0, 0], dtype=int)] , data[k, 0, 0, 0, :], np.array(data[k, :, index1, index2, :]).T , shading='auto', norm=colors.LogNorm(vmin=np.array(data[k, :, index1, index2, :]).T.min(), vmax=np.array(data[k, :, index1, index2, :]).T.max()*1e-1), cmap='winter_r' ) #selects average time, frequencies, and then the periodogram 
                else:
                     img = axes[2*i+k + length_for_axis].pcolormesh(datetimes[np.array(data[k, :, 4,0, 0], dtype=int)] , data[k, 0, 0, 0, :], np.array(data[k, :, index1, index2, :]).T , shading='auto', norm=colors.LogNorm(vmin=np.array(data[k, :, index1, index2, :]).T.min()*1e2, vmax=np.array(data[k, :, index1, index2, :]).T.max()), cmap='winter' ) #selects average time, frequencies, and then the periodogram 
                
                fig.colorbar(img, ax=axes[2*i+k+length_for_axis], extend='max', label=query_dict["heatmap"][i])
                axes[2*i+k + length_for_axis].set_ylabel(
                    "Frequencies"
                )
                axes[2*i+k + length_for_axis].set_xlabel(
                    "Times"
                )
                axes[2*i+k + length_for_axis].set_title(
    ...
    def graphingE(label, arrayx, arrayy, arraybandy, satelliteindex, has_twin):
        ...
        for i in range(len(query_dict["graph_E_chosen"])):
            ...
            if has_twin==True:
                ...
                global axes_twin_E
                axes_twin_E=axes[i].twinx()

            if query_dict["bandpass"][0] == True:
                axes_twin_E.plot(arrayx,arraybandy[:, index], label="".join([label, "bandpassed"]), color=colors[satelliteindex+3])
                axes_twin_E.set_ylabel(
                r"$E_{{{}}}$ bandpassed $(mV/m)$".format(query_dict["graph_E_chosen"][i])
            )
                axes_twin_E.legend(loc=1)
        return True
    ...
    def graphingB(label, arrayx, arrayy, arraybandy, satelliteindex, has_twin):
        ...
        for i in range(len(query_dict["graph_B_chosen"])):
            ...
            if has_twin==True:
                ...
                global axes_twin_B
                axes_twin_B=axes[i +length_for_axis].twinx()

            if query_dict["bandpass"][0] == True:
                axes_twin_B.plot(arrayx,arraybandy[:, index], label="".join([label, "bandpassed"]), color=colors[satelliteindex+3])
                axes_twin_B.set_ylabel(
                r"$B_{{{}}}$ bandpassed $(nT)$".format(query_dict["graph_B_chosen"][i])
            )
                axes_twin_B.legend(loc=1)
        return True
    ...
    def graphingFlux(label, arrayx, arrayy, index_band, satelliteindex, has_twin):
        ...
        for i in range(len(query_dict["graph_PF_chosen"])):
            ...
            if has_twin==True:
                ...
                global axes_twin_PF
                axes_twin_PF=axes[i + length_for_axis].twinx()
            try:
                if index_band==0:
                    ...
                    axes_twin_PF.plot(arrayx,arrayy[index], label="".join([label, "bandpassed"]), color=colors[satelliteindex+3])
                ...
                if index_band==0:
                    ...
                    axes_twin_PF.plot(arrayx,arrayy[index], label="".join([label, "bandpassed"]),  colors=colors[satelliteindex+3])
                    axes_twin_PF.legend(loc=1)
