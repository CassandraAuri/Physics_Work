...
for i in range(2):
    ...
    print(data[i, 0,  3, 0, bandpass][0])
    print(data[i, 0,  3, 1, bandpass][0]) 
    print("Conductivies of each satlelite from each polarization")
    conducitivies.append(1/(1.256e-6*data[i, 0,  3, 0, bandpass][0]))
    Alfven_speed.append(np.mean(np.absolute(data[i, 0,  3, 0, bandpass])))
powerspec =  np.absolute(data[i, 0, 3, 0, bandpass])
...
def my_loglike(theta, data):
    model_init = model(
        theta[0],
        theta[1]*300e3*np.sqrt(theta[4]**2+np.exp((-theta[2]*100e3/theta[3]))),
        theta[2] * 100e3,
        theta[3],
        theta[4],
        0.025, #?
...
def main():
    ...
    with pm.Model():
        ...
        sigmap = pm.Uniform("sigmap", lower=1, upper=5, initval=sigmap[0])

        VA_norm = pm.Uniform("vai", lower=0.5, upper=2, initval=Alfven_speed[0]/300e3)
        h_norm = pm.Uniform("h", lower=0.5, upper=3, initval=1)
        # convert m and c to a tensor vector
        theta = pt.as_tensor_variable([sigmap, VA_norm, h_norm, 400e3, 0.01])
